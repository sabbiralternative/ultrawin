  ...
  return (
    <>
      {showNotification && filteredNotification?.length > 0 && (
        <div className="w-full relative max-h-[32px] bg-bg_appBg overflow-hidden">
          <div
          ...
          >
            {filteredNotification?.map((item) => (
              <span
                key={item?.id}
                className="flex items-center relative  gap-x-1  transition-all ease-in-out duration-200 p-1 rounded-lg min-w-max whitespace-nowrap text-nowrap bg-bg_text_brand_primary text-transparent bg-clip-text mr-[50vw]"
              >
                {item?.text}
              </span>
            ))}
